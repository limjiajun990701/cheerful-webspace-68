...
const MySkill = () => {
  ...
  // Fetch skill groups on mount
  useEffect(() => {
    const fetchSkillGroups = async () => {
      setIsLoading(true);
      const data = await getSkillGroups();
      setSkillGroups(data || []);
      // Select the first group if exists
      ...
      >
        ...
        {skillGroups.map(group => (
          <TabsContent key={group.id} value={group.id}>
            {group.description && (
              <p className="text-center text-muted-foreground mb-8">{group.description}</p>
            )}

            {/* Skill Items List (horizontal scroll box) */}
              ...
              <div className="w-full py-6 bg-secondary/10 overflow-x-auto rounded-xl">
                <div className="flex gap-4 px-4 min-w-max">
                  {group.items.map((item, idx) => (
                    <Card
                      key={`${group.id}-${idx}`}
                      className="flex flex-col items-center w-[160px] min-w-[140px] px-4 py-4 border-none bg-background shadow"
                    >
                      ...
                      {item.description && (
                        <div className="text-xs text-muted-foreground mt-1 text-center">{item.description}</div>
                      )}
                    </Card>
                  ))}
                </div>
